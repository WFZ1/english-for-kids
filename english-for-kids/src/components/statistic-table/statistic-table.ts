...
export default class StatisticTable extends BaseComponent {
  ...
  private sortData(e: MouseEvent): void {
    const target = getContainEl(e.target as HTMLElement, 'th', this.statisticTableHead.el);

    if (target) {
      this.statisticTableHead.handleSort(target);

      const colIndex = this.statisticTableHead.statisticTableRow.cells.findIndex((cell) => cell.el === target);
      const sortType = target.classList.contains(STATISTIC_CELL_SORT_CLASSES.descend) ? 'descend' : 'ascend';
