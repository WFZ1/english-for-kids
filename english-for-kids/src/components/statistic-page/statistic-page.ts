...
import {
  ...
  BTN_REPEAT_DIFFICULT_WORDS_TEXT,
  BTN_RESET_STATISTIC_TEXT,
  DIFFICULT_WORDS_ACCURACY_LIMIT,
  DIFFICULT_WORDS_ACCURACY_TEXT,
  MAX_DIFFICULT_WORDS,
  STATISTIC_PAGE_URL,
...
export default class StatisticPage {
  ...
  constructor(private readonly rootEl: HTMLElement) {
    ...
    this.btnRepeat = new Btn({
      classes: ['statistic-page__btn', 'statistic-page__repeat'],
      text: BTN_REPEAT_DIFFICULT_WORDS_TEXT,
      title: DIFFICULT_WORDS_ACCURACY_TEXT,
    });
    this.btnReset = new Btn({
      classes: ['statistic-page__btn', 'statistic-page__reset'],
      text: BTN_RESET_STATISTIC_TEXT,
    });
  ...
  private handleDifficultWords(): void {
    ...
    const filteredRows = StatisticPage.filterDifficultWords(
      sortedRows,
      indexHitCol,
    );
    ...
    store.dispatch({
      type: APP_DIFFICULT_CATEGORY,
      difficultWords: structuredWords,
    });
  }

  private static filterDifficultWords(
    ...
    const filteredRows = rows.filter((row) => {
      ...
      if (
        Number(cellsVal.hit) < DIFFICULT_WORDS_ACCURACY_LIMIT &&
        cellsVal.wrong !== '0' &&
        maxRows
      ) {
        maxRows--;
  ...
  private static structureDifficultWords(
    rows: StatisticTableRow[],
  ): ICategoryWord[] {
    return rows.map((row) => {
      const obj = {
