...
export default class AdminCategoryCard extends BaseComponent {
  ...
  constructor(categoryProps: IAdminCategoryCardProps) {
    ...
    this.adminCategoryCardForm = new AdminCategoryCardForm({ formClasses: ['admin-category-card__form-update', 'admin-category-card-form_hidden'], formAction: SERVER_API_CATEGORY_URL, formMethod: 'PATCH', inputValue: categoryProps.title, btnClasses: ['admin-category-card__btn'], btnCancelClass: 'admin-category-card__btn-cancel-update', btnActClass: 'admin-category-card__btn-save-update', btnActText: 'Save' });
  ...
  private build(categoryProps: IAdminCategoryCardProps): void {
    ...
    this.el.append(this.btnRemove.el, this.containerEl, this.adminCategoryCardForm.el);
    this.containerEl.append(this.titleEl, this.amountWordsEl, this.btnUpdate.el, this.linkToWord.el);
